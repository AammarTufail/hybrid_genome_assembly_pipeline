...
bakta_fna="$bakta_out/all_fna"
mkdir -p $bakta_out $bakta_gff $bakta_fna
...
do
  ...
  if (( counter % num_files_short == 0 )); then
    wait
    counter=0
  fi
done
...
do
    ...
    if (( counter % num_files_long == 0 )); then
      wait
      counter=0
    fi
done
...
echo -e "Assemblies without canu corrected long reads:= $not_corrected_reads" >> $report
printf "\n" >> $report
...
export CHECKM2DB="$checkm2_db_dir/CheckM2_database/uniref100.KO.1"
...
# Copy all gff and fna files into repctive dirs
cd $bakta_out || exit 1
find ./ -type f -name "*.gff3" -exec cp {} $bakta_gff \;
find ./ -type f -name "*.fna" -exec cp {} $bakta_fna \;
...
gtdbtk classify_wf --extension fasta --genome_dir $all_assemblies_fasta --out_dir $gtdb_out --cpus $threads --pplacer_cpus 1
  # pplacer issues can arise with insufficient memory, therefore reduced threads for pplacer
    # In testing it needed at least 64 GB
